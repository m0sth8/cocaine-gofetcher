...
# Execute "go install"
MESSAGE (STATUS "Executing: ${GO_EXECUTABLE} build -a -tags=\"${GOTAGS}\" -gcflags=\"${GCFLAGS}\" -ldflags=\"${LDFLAGS}\" ${_go_debug} ${PACKAGE}")
EXECUTE_PROCESS (RESULT_VARIABLE _failure
  COMMAND "${GO_EXECUTABLE}" build "-tags=${GOTAGS}" "-gcflags=${GCFLAGS}" "-ldflags=${LDFLAGS}" ${_go_debug} "${PACKAGE}")
IF (_failure)
  MESSAGE (STATUS "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
  MESSAGE (STATUS "@ 'go build' failed! Re-running as manual go build to help debug...")
  MESSAGE (STATUS "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
ENDIF (_failure)
...
# is unavoidable.
IF (OUTPUT)
  FILE (RENAME ${CMAKE_CURRENT_BINARY_DIR}/${PKGEXE} ${CMAKE_CURRENT_BINARY_DIR}/${OUTPUT})
ENDIF (OUTPUT)
